...
### 프로필은 마지막 로그인 제공자를 따라간다

같은 이메일이면 카카오·구글이 한 계정으로 묶이는데, 프로필은 가입 때
한 번만 채워져 처음 가입한 제공자의 사진·이름이 계속 남았다.
`20260804000001` 의 `sync_profile_on_login()` 트리거가 로그인할 때마다
사진·닉네임을 그 제공자의 값으로 바꾼다.

- `raw_user_meta_data` 는 제공자 키가 **병합**돼 남아 어느 쪽 값인지 구분이
  안 된다. 대신 `auth.identities.identity_data`(제공자 순수 데이터)를 쓰고,
  `last_sign_in_at` 이 갱신되는 행으로 방금 쓰인 제공자를 안다.
- 제공자가 안 주는 값은 있던 값을 지우지 않는다.
