...
### 아바타는 마지막 로그인 제공자를 따라간다

같은 이메일이면 카카오·구글이 한 계정으로 묶이는데, 프로필은 가입 때
한 번만 채워져 처음 가입한 제공자의 사진이 계속 남았다.
`20260804000001` 의 `sync_avatar_on_login()` 트리거가 로그인으로
`raw_user_meta_data` 가 바뀔 때 아바타를 따라 바꾼다.

- **닉네임은 따라가지 않는다** — 구글 `full_name` 은 "seokachu (서카츄)"
  처럼 이메일 표기가 섞여 와서 오히려 나빠진다.
- 새 제공자가 사진을 안 주면 있던 사진을 지우지 않는다.
