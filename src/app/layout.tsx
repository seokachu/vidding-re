 ...
 *
 * **단, 터치 기기는 폭 제한을 풀어 화면을 양옆까지 채운다.** 폴드를 펼치거나
 * 아이패드로 열면 390px 열이 가운데 덩그러니 남는데, 손에 든 기기에서는 그게
 * 여백이 아니라 빈 공간이다. `--shell-width`(globals.css) 가 `pointer: coarse`
 * 에서 `100%` 로 바뀌어 셸 · 하단 탭 · 배너 · 토스트가 함께 넓어진다 — 글자
 * 크기는 그대로, 카드 · 탭 · 버튼만 늘어난다. 마우스로 보는 웹은 그대로 390px 다.
 *
