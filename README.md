...
화면 32장과 디자인 시스템 4장을 배포된 서비스 안에서 그대로 열람할 수 있다.
...
- 세션 유지, 외부 앱 스킴 열기, 당겨서 새로고침(채팅방은 예외)
- 연결이 끊기면 웹뷰 내장 에러 페이지 대신 우리 오프라인 화면이 뜨고, 연결이 돌아오면
  손대지 않아도 다시 불러온다
- 스플래시는 네이티브 단계에서 배경색만 보이고 RN 이 로고 + 태그라인을 잇는다 — 로고가 튀지 않는다
- Android 뒤로가기는 두 층 규칙이다 — 하단 탭(1층)에서는 두 번 눌러 종료, 그 밖의 화면은
  웹 히스토리 back. 탭 전환은 히스토리에 쌓지 않아 뒤로가기가 끝없이 이어지지 않는다
...
- 데스크톱 브라우저는 390px 열을 가운데 두고, 터치 기기(폰 · 펼친 폴드 · 아이패드)는
  화면 폭을 그대로 채운다 — 글자 크기는 그대로, 카드 · 탭 · 버튼만 늘어난다
- 공통 UI 20종을 [스토리북](https://vidding-storybook.vercel.app)으로 문서화했다

**에러 · 오프라인**
- 없는 주소(404) · 끊긴 연결(오프라인) · 그리다 터진 화면(500) 셋 다 갈 곳을 함께 준다 —
  서비스 밖으로 튕겨 나가는 자리를 없앴다
- 서비스 워커는 오프라인 폴백 한 장만 미리 받아 두고 페이지는 캐시하지 않는다 —
  "고쳤는데 옛 화면이 뜬다"를 만들지 않는다
- 접근 판정은 실제로 있는 화면에만 한다 — 없는 주소는 비회원도 로그인이 아니라 404 를 본다.
  아는 경로 목록은 CI 가 `src/app` 과 대조한다

**살아 있는 목록**
- GitHub Actions 크론이 하루 1건 새 경매를 올리고, 진행중 경매에 사연 · 입찰 · 공감을
  며칠에 걸쳐 나눠 채운다
- 풀이 비면 마감된 지 7일 지난 경매를 다시 올린다 — 손대지 않아도 무한히 돈다.
  테이블에 직접 쓰지 않고 실제 사용자 흐름(RPC · RLS)을 그대로 밟는다
...
- APK 는 약 27 MB (arm64 전용). 크롬 다운로드 화면이 100% 에서 멈춘 듯 보여도 이미 받아진 것이니
  알림창에서 `vidding.apk` 를 열면 된다
...
| [디자인 시안 (웹)](https://vidding-re.vercel.app/docs/design) | `.pen` 화면 32장 · 디자인 시스템 4장 |
